...
from src.data import load_gift_data, gift_data_split, gift_data_loader
...
def main(args):
    ...
    wandb.init(id = wandb_id, resume = "allow", project= args.project, name = f'{args.model}_{timestamp}', config = config)
    ...
    if args.model in ('FM'):
        data = gift_data_split(args, data)
        data = gift_data_loader(args, data)
...
if __name__ == "__main__":
    ...
    # WANDB OPTION
    arg('--project', type=str, default='gifthub', help='프로젝트 이름을 설정할 수 있습니다.')
    ...
    arg('--model', type=str, default = 'WDN', choices=['FM', 'FFM', 'NCF', 'WDN', 'DCN', 'CNN_FM', 'DeepCoNN', 'ROP_CNN'],
