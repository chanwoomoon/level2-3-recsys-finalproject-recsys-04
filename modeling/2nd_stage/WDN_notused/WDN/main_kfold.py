...
import numpy as np
from src.train.trainer import train, test, valid, RMSELoss
...
def main(args):
    ...
    elif args.model in ['DeepCoNN', 'ROP_CNN']:
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(data['train'])):
        print(f"Training on fold {fold+1}/{args.kfolds}")

        # 훈련 및 검증 데이터셋 분리
        train_data = Subset(data['train'], train_idx)
        valid_data = Subset(data['train'], valid_idx)

        # DataLoader 생성
        train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True)
        valid_loader = DataLoader(valid_data, batch_size=args.batch_size, shuffle=False)
        dataloaders = {'train_dataloader': train_loader, 'valid_dataloader': valid_loader}

        # 모델 훈련
        model = models_load(args, train_data)
        trained_model = train(args, model, dataloaders, logger, setting)

        # 검증 및 결과 기록
        valid_loss = valid(args, trained_model, dataloaders['valid_dataloader'], RMSELoss() if args.loss_fn == 'RMSE' else MSELoss())
...
if __name__ == "__main__":
    ...
    # siyun - 추가적인 kfold 옵션
    arg('--kfolds', type=int, default=5, help='Number of folds for K-Fold cross-validation')
